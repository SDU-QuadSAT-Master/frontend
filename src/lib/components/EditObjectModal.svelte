<script lang="ts">
  ...
  const getId = (): string => {
    switch (objectType) {
      case 'Mission':
        return objectData.missionId
      case 'Drone':
        return objectData.droneId
      case 'Antenna':
        return objectData.antennaId
      case 'Scan Plan':
...
<Modal title="Edit Object" bind:open autoclose={false} size="lg">
  <form>
    ...
    {#if objectType === 'Scan Plan' || objectType === 'Calibration Plan'}
      <div class="grid gap-6 mb-6 md:grid-cols-2">
        <div>
          ...
          <Input type="text" id="name" bind:value={objectData.name} required />
        </div>
        <div>
        ...
        {#if objectType === 'Calibration Plan'}
          ...
          {#if objectData.type === 1}
            ...
            <div>
              <Label for="zenith" class="mb-2">Zenith</Label>
    ...
    <Button
      on:click={() => {
        open = false
        let id = getId()
        onUpdate(Object.assign({}, objectData, { id }))
        objectData = {}
      }}>Save Changes</Button
    >
  </form>
